...
def show_page():
    ...
    st.markdown(f"""
        <div style='text-align: center; background-color: #F0F8FF; padding: 30px; border-radius: 20px; border: 3px dashed #BAE1FF;'>
            <h1 style='color: #0056b3;'>🗺️ {REGION} 탐험대에 오신 것을 환영합니다!</h1>
            <p style='font-size: 18px; color: #333;'>우리 동네 {REGION}의 숨겨진 비밀과 학교의 역사를 찾아 떠나는 신나는 여행!</p>
            <p style='font-size: 17px; color: #E65100; font-weight: bold; margin-top: 15px;'>확인이 끝나면 왼쪽 메뉴 선택 중 로그인이나 회원가입을 눌러 여행을 시작해요!</p>
        </div>
        <br>
    """, unsafe_allow_html=True)

    st.write(f"{REGION} 탐험대 대원 여러분! 왼쪽 메뉴를 눌러 아래의 3가지 탐험을 순서대로 진행해 보세요. 🚀")
    ...
    st.info(
        "💡 **도움이 필요할 때는?**\n\n"
        "각 활동 화면 아래에는 **'🤖 AI 보조교사'**가 기다리고 있어요. "
        "모르는 것이 있거나 무엇을 해야 할지 헷갈릴 때는 언제든 질문해 주세요!"
    )

    st.warning(
        "🕵️ **탐험대원이 꼭 기억할 약속**\n\n"
        "AI는 아주 똑똑하지만 **모르는 것도 있고, 가끔 틀린 이야기를 하기도 해요.**\n\n"
        "AI가 알려준 것을 그대로 믿지 말고, 박물관 자료를 찾아보거나 "
        "가족과 선생님께 여쭤보며 **진짜인지 확인하는 탐정**이 되어 주세요!"
    )
