...
if db_connected:
    ...
    col_2_1 = db["act2_1"]
    col_2_2 = db["ai_detective_log"]          # AI 유물 탐정 질문 기록
...
def show_page(target_student=None):
    ...
    st.markdown("""
        ...
        .check-note { display: inline-block; margin-top: 8px; font-size: 0.9rem; color: #2E7D32; background-color: #E8F5E9; padding: 4px 10px; border-radius: 8px; font-weight: bold; }
    ...
    st.markdown(f"""
        <div class="dash-header">
            ...
            <div class="dash-subtitle">지금까지 {REGION} 탐험대에서 남긴 모든 발자국을 모아봤어요!</div>
        </div>
    """, unsafe_allow_html=True)
    ...
    try:
        ...
        count_2_2 = col_2_2.count_documents({"username": current_student})
        ...
        print(f"[DB COUNT ERROR] stu_dash: {e}")
        st.warning("⚠️ 기록을 세는 중에 문제가 생겼어요. 잠시 후 다시 들어와 볼까요?")
        return

    rec_1_3_count = 0 if target_student else len(st.session_state.get("my_secret_timeline", []))
    total_records = (
        count_1_1 + count_1_2 + rec_1_3_count + count_2_1 + count_2_2
        + count_2_3 + count_3_1 + count_3_2 + count_3_3
    )
    ...
    try:
        ...
        rec_2_2 = list(col_2_2.find({"username": current_student}).sort("_id", -1).limit(10))
        ...
        print(f"[DB READ ERROR] stu_dash: {e}")
        st.warning("⚠️ 기록을 불러오는 중에 문제가 생겼어요.")
        return
            ...
            def get_photo_box(stage_data, box_class, title_text):
                if not stage_data:
                    return (
                        ...
                        f'<div class="box {box_class}" style="color: #ccc;">기록 없음</div>'
                        f'<div style="margin-top: 3px; font-weight: bold; text-align: center; line-height: 1.2;">{title_text}</div></div>'
                    )
                img_base64 = stage_data.get("image_base64", "")
                content_text = stage_data.get("content", "")

                # 사진 없이 글만 저장한 경우도 있으므로 나눠서 그립니다.
                if img_base64:
                    box_inner = (
                        f'<div class="box {box_class}" style="padding: 0; overflow: hidden; display: flex; '
                        f'justify-content: center; align-items: center; border-width: 5px;">'
                        f'<img src="data:image/jpeg;base64,{img_base64}" '
                        f'style="width: 100%; height: 100%; object-fit: cover; border-radius: 18px;"></div>'
                    )
                else:
                    box_inner = f'<div class="box {box_class}" style="font-size:0.95rem; color:#888;">📝 글로만<br>기록했어요</div>'

                return (
                    f'<div style="display: flex; flex-direction: column; align-items: center;">'
                    f'{box_inner}'
                    f'<div style="margin-top: 5px; color: black; font-weight: bold; text-align: center; line-height: 1.2;">{title_text}</div>'
                    f'<div class="memory-bubble">{content_text}</div></div>'
                )
                ...
                with st.container():
                    ...
                    if item.get("image_base64"):
                        try:
                            ...
                            print(f"[IMAGE ERROR] stu_dash 1_2: {e}")
                    st.markdown('</div>', unsafe_allow_html=True)

    with tab1_3:
        if target_student:
            st.warning("🔒 1-3 발자국 연표는 학생 개인 화면에만 임시로 저장되는 '비밀 일기'이므로 선생님 화면에서는 보이지 않습니다.")
        else:
            ...
            if len(my_timeline) == 0:
                ...
                st.caption("💾 비밀 연표는 서버에 저장되지 않아요. 활동 화면에서 파일로 내려받아 보관하세요!")
    ...
    with tab2_1:
        if count_2_1 == 0:
            ...
            # AI 분석 내용과 사진은 DB에 저장하지 않습니다. (저작권·용량 고려)
            # 어떤 물건에 대한 생각인지 알 수 있도록 이름은 함께 보여줍니다.
            for item in rec_2_1:
                with st.container():
                    st.markdown('<div class="record-card">', unsafe_allow_html=True)
                    st.markdown('<span class="record-tag">옛 물건 살펴보기</span>', unsafe_allow_html=True)
                    relic = item.get("relic_name")
                    if relic:
                        st.markdown(f'<div class="record-title">🏷️ {relic}</div>', unsafe_allow_html=True)
                    st.markdown(
                        f'<div class="record-content"><b>✍️ 나의 생각:</b> "{item.get("thought", "")}"</div>',
                        unsafe_allow_html=True,
                    )
                    st.markdown('</div>', unsafe_allow_html=True)
    ...
    with tab2_2:
        if count_2_2 == 0:
            st.info("아직 AI 유물 탐정에게 질문한 기록이 없어요.")
        else:
            st.success(f"💬 AI 유물 탐정에게 {count_2_2}번 질문했어요! 궁금한 것을 스스로 찾아본 멋진 기록입니다.")
            for item in rec_2_2:
                with st.container():
                    st.markdown('<div class="record-card">', unsafe_allow_html=True)
                    st.markdown('<span class="record-tag">AI 유물 탐정</span>', unsafe_allow_html=True)
                    st.markdown(f'<div class="record-title">❓ {item.get("question", "")}</div>', unsafe_allow_html=True)
                    st.markdown(f'<div class="record-content">{item.get("answer", "")}</div>', unsafe_allow_html=True)
                    st.markdown('</div>', unsafe_allow_html=True)
        ...
        if count_2_3 == 0:
            ...
            for item in rec_2_3:
                with st.container():
                    ...
                    st.markdown(
                        f'<div class="record-content" style="margin-bottom:10px;">'
                        f'👤 주인: {item.get("item_owner", "")} | ⏳ {item.get("item_era", "")}</div>',
                        unsafe_allow_html=True,
                    )
                    ...
                    if item.get("image_base64"):
                        try:
                            ...
                            print(f"[IMAGE ERROR] stu_dash 2_3: {e}")
                    st.markdown('</div>', unsafe_allow_html=True)
    ...
    with tab3_1:
        if count_3_1 == 0:
            st.info(f"아직 '{REGION}의 옛이야기 탐험' 활동을 기록하지 않았어요.")
        else:
            for item in rec_3_1:
                with st.container():
                    st.markdown('<div class="record-card">', unsafe_allow_html=True)
                    ...
                    if item.get("check_method"):
                        st.markdown(f'<div class="check-note">🕵️ 확인 방법: {item["check_method"]}</div>', unsafe_allow_html=True)
                    st.markdown('</div>', unsafe_allow_html=True)

    with tab3_2:
        if count_3_2 == 0:
            st.info(f"아직 '{REGION}의 달라진 모습' 활동을 기록하지 않았어요.")
        else:
            for item in rec_3_2:
                with st.container():
                    st.markdown('<div class="record-card">', unsafe_allow_html=True)
                    ...
                    if item.get("place"):
                        st.markdown(f'<div class="record-title">📍 {item["place"]}</div>', unsafe_allow_html=True)
                    ...
                    st.markdown(
                        f'<div class="record-content" style="margin-top:10px;">'
                        f'<b>🤔 달라진 이유:</b> {item.get("reason", "")}</div>',
                        unsafe_allow_html=True,
                    )
                    st.markdown('</div>', unsafe_allow_html=True)
    ...
    with tab3_3:
        if count_3_3 == 0:
            st.info(f"아직 '{REGION}의 땅 이름 비밀 찾기' 활동을 기록하지 않았어요.")
        else:
            for item in rec_3_3:
                with st.container():
                    st.markdown('<div class="record-card">', unsafe_allow_html=True)
