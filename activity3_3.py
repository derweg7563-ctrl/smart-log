...
def show_page():
    ...
    st.markdown("""
        ...
        div[data-testid="column"]:has(.padlet-btn-hook) button {

            height: 70px !important;

            border-radius: 25px !important;

            background: linear-gradient(135deg, #81D4FA, #0288D1) !important;
        ...
        div[data-testid="column"]:has(.padlet-btn-hook) button:hover {
    ...
    # 이 활동을 마무리하면서 버튼 (패들릿 링크)

    col_empty4, col_btn3, col_empty5 = st.columns([1, 1.5, 1])

    

    with col_btn3:

        st.markdown("<span class='padlet-btn-hook'></span>", unsafe_allow_html=True)

        st.link_button(

            "✏️ 이 활동을 마무리하면서",

            "https://padlet.com/lehrer00/padlet-bsilay67l130ev32",
